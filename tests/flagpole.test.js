...
test('flag -> level-clear: drains timer into score, EXACT total, then advances (via intro)', () => {
  const gs = createGameState({ worldFactory: worldFactory(5), levelCount: 2, scriptTimes: CLEAR, introTime: 0 });
  ...
  assertEqual(gs.state, STATES.intro, 'next level shows the intro card');
});
...
test('level-clear lasts ~its configured duration, NOT timer-rate', () => {
  ...
  const gs = createGameState({ worldFactory: worldFactory(300), levelCount: 2, scriptTimes: CLEAR, introTime: 0 });
  gs.startGame(); toPlay(gs);
  gs.world.flagReached = true; gs.update(1/60, NONE);
  let steps = 0;
...
test('dying script runs its duration then reloads (via intro)', () => {
  const gs = createGameState({ worldFactory: worldFactory(5), levelCount: 2, scriptTimes: CLEAR, introTime: 0 });
  gs.startGame(); toPlay(gs); gs.session.lives = 2;
  ...
  assertEqual(gs.state, STATES.intro, 'reloads via the intro card');
});
